...
In a CS, the population is divided into clusters (PSUs), a subset of clusters is sampled, and then either all units or a subsample of units within selected clusters are observed. 
CS is mainly used to reduce costs, often at the expense of higher variance due to intra-cluster similarity.
...
... so it is a natural variation who affects the variance (precision) of estimates , BUT, it can be quantified (SE and CI) and reduced by increasing sample size and/or improving design!
-Measurement error arises from inaccuracies in data collection, so is often systematic (poorly worded questions, respondent misreporting, interviewer effects, recall bias). 
It is a non-sampling error, it introduces bias, and does not disappear with larger samples! In addition, it is difficult to quantify because depends on this own specificities

3) combinatory calculus, to derive probabilities of selection?
  Combinatory calculus enumerates all possible samples that can be drawn from a population under a given design and also it calculates how often a given unit or cluster appears across these samples! 
  --> These allows us to compute INCLUSION PROBABILITY for samples, clusters, and individual units. So, it Is essential for: unbiased estimation and variance calculation.

4) SRS in a small vs large population
- in small N, sampling fractions can be large‼ so sampling WOR is not good, and  fcp factor is important; estimates become more precise as the sample approaches a census.
- in large N, the sampling fraction is typically very small, dependence between draws is negligible (WOR formulas could be used) , the fpc is close to 1

5) FPC: definition and use
FPC adjusts variance estimates to account for sampling WOR from a finite population and is typically expressed as (N− n)/N or using Bessel-->  (N − n)/(N − 1) 
...
Once the mean is fixed, the data points are no longer all “free” to vary independently: knowing n – 1 values automatically “forced” the last one! (degrees of freedom theory) 
...
-->  SYS induces implicit STRATIFICATION, often reducing variance by guaranteeing representation  -->  more precise than SRS‼ (because observations are well spread across the range)
